		...
		unsigned int xid_rep, char **b, int *sz)
{
    ...
    rpcstate_t ret = NEW;
    if (!reply_window_[clt_nonce].empty() && xid <= reply_window_[clt_nonce].front().xid) {
        ret = FORGOTTEN;
    ...
    if (it == reply_window_[clt_nonce].end() && ret == NEW) {
    ...
    if (reply_window_[clt_nonce].empty() || reply_window_[clt_nonce].front().xid > xid_rep) {
        reply_window_[clt_nonce].push_front(reply_t(xid_rep));
    }
...
{
	ScopedLock rwl(&reply_window_m_);
    std::map<unsigned int, std::list<reply_t>>::iterator mit;
    std::list<reply_t>::iterator it;
    mit = reply_window_.find(clt_nonce);
    if (mit != reply_window_.end()) {
